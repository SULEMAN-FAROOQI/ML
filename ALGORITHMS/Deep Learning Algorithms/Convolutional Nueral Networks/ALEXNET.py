...
# meta["X_shape_"][1:] → (28, 28, 1) — you slice off the batch dimension, since Keras's input_shape argument wants the shape of a single sample, not the whole batch.
    ...
    m.add(Conv2D(96, kernel_size=(11,11), padding="same", activation="relu", strides=4, input_shape = input_shape))
    m.add(BatchNormalization())
    m.add(MaxPool2D(pool_size=(3,3), padding = "valid", strides = 2))

    m.add(Conv2D(256, kernel_size=(5,5), padding="same", activation="relu"))
    m.add(BatchNormalization())
    m.add(MaxPool2D(pool_size=(3,3), padding = "valid", strides = 2))
...
# ALEXNET was originally trained on 14 million images but we are only training on 5k images so the model overfits. 
# I added batch normilization to increase training accuracy.