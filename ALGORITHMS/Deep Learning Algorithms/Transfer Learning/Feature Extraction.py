...
def build_model(meta):
    ...
    m.add(Dropout(0.3))
    m.add(Dense(32, activation="relu", kernel_regularizer=l2(0.005)))
    m.add(Dropout(0.3))
...
m = KerasClassifier(model=build_model, epochs=10, batch_size=33, verbose=1, validation_split=0.3, callbacks=[callback])
